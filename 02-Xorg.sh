...
extrasrc="http://repository.slacky.eu/slackware-13.0/"
...
mkdir -p $NP $NP-work $NP-removed/man_pages/usr/man $NP-removed/locale/usr/share/locale $NP-removed/devel/usr/{include,lib${ARCH}/qt/include}

downloadpkg() {
cd $SD/$NP-work
wget -R "$blacklist_x" "$slacksrc"/x/*.txz
wget -A "$whitelist_l" "$slacksrc"/l/*.txz

wget ftp://ftp.iasi.roedu.net/mirrors/ftp.slackware.com/pub/slackware/slackware${ARCH}-current/extra/google-chrome/GConf-*.txz
wget ftp://ftp.iasi.roedu.net/mirrors/ftp.slackware.com/pub/slackware/slackware${ARCH}-current/extra/google-chrome/ORBit2-*.txz
}
...
rm usr/lib${ARCH}/xorg/modules/dri/{mach64_dri.so,tdfx_dri.so}

# Make a separate DRI package
...
if [[ -z $1 ]]; then
	...
	case $1 in
	 ...
	 "lzmfy" )
	  ...
	  mksquashfs $NP-3D $NP-3D.lzm -noappend -b 256k
	 ;;
	 "world" )
	  echo "...DOWNLOADING"
	  downloadpkg
